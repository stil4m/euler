...
main =
    input
        |> allForms 0
        |> List.concat
        |> List.map (List.Extra.groupsOfWithStep 4 1)
        |> List.concat
...
allForms : a -> Grid a -> List (Grid a)
allForms fill x =
    let
        diagonalA =
            x |> List.indexedMap (\n v -> List.repeat n fill ++ v)
        ...
        diagonalB =
            x
                ...
                |> List.reverse
    in
    [ x, flip x, flip diagonalA, flip diagonalB ]
